...
Sys._Application = function Sys$_Application() {
    ...
    function Sys$_Application$get_stateString() {
        /// <value type="String" locid="P:J#Sys._Application.stateString"></value>
        if (arguments.length !== 0) throw Error.parameterCount();
        ...
        // before returning it.
        // The fix, which is unfortunately not entirely fool-proof,
        // is to deserialize without decoding and reserialize the hash.
    ...
    function Sys$_Application$add_navigate(handler) {
        /// <summary locid="E:J#Sys._Application.navigate" />
        var e = Function._validateParams(arguments, [{name: "handler", type: Function}]);
        if (e) throw e;
    ...
    function Sys$_Application$addHistoryPoint(state, title) {
        /// <summary locid="M:J#Sys._Application.addHistoryPoint" />
        /// <param name="state" type="Object"></param>
    ...
    function Sys$_Application$setServerId(clientId, uniqueId) {
        /// <summary locid="M:J#Sys._Application.setServerId" />
    ...
    function Sys$_Application$setServerState(value) {
        /// <summary locid="M:J#Sys._Application.setServerState" />
        /// <param name="value" type="String"></param>
        var e = Function._validateParams(arguments, [
            {name: "value", type: String}
        ]);
    ...
    function Sys$_Application$_deserializeState(entry, skipDecodeUri) {
        ...
        // <param name="skipDecodeUri" type="Boolean" optional="true">If true, decodeURIComponent will not be applied to the values.</returns>
...
//                Sys.Debug.trace("posting back.");
                __doPostBack(this._uniqueId, newServerEntry);
                this._state = state;
                return;
            }
        }
//        Sys.Debug.trace("Navigate setting state to " + entry);
...
//            Sys.Debug.trace("entry changed from " + this._currentEntry + " to " + entry);
    ...
    function Sys$_Application$_onIFrameLoad(entry) {
        this._ensureHistory();
//        Sys.Debug.trace("iFrame load " + !this._ignoreIFrame + ":" + entry);
...
//            Sys.Debug.trace("Server sent " + dataItem);
        ...
        if (entry !== this._currentEntry) {
//            Sys.Debug.trace("Server changed state from " + this._currentEntry + " to " + entry);
            this._ignoreTimer = true;
            this._setState(entry);
            this._raiseNavigate();
        }
...
//                Sys.Debug.trace("Hash is changing from " + currentHash + " to " + entry);
                if (this._isSafari2()) {
                    // Safari 2 doesn't update the location.hash when the user hits back.
...
Sys.UI.PermaLink = function Sys$UI$PermaLink(element) {
    Sys.UI.PermaLink.initializeBase(this,[element]);
}

    function Sys$UI$PermaLink$dispose() {
    ...
    function Sys$UI$PermaLink$initialize() {
    ...
    function Sys$UI$PermaLink$_onNavigate(sender, args) {
        this.get_element().href = '#' + Sys.Application.get_stateString();
    }
Sys.UI.PermaLink.prototype = {
    dispose: Sys$UI$PermaLink$dispose,
    initialize: Sys$UI$PermaLink$initialize,
    _onNavigate: Sys$UI$PermaLink$_onNavigate
}
