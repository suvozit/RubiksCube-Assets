...
function BU()
{ var w = window; if (!w.lqm_pageid) w.lqm_pageid = gR(); w.lqm_url = location.protocol + "//a.lakequincy.com/i.ashx?"; if (!CMA()) return; QS("channel", w.lqm_channel); QS("country", w.lqm_country); QS("debug", w.lqm_debug); QS("format", w.lqm_format); QS("height", w.lqm_height); QS("placement", w.lqm_placement); QS("creative", w.lqm_creative); QS("pageid", w.lqm_pageid); QS("publisher", w.lqm_publisher); QS("servertime", w.lqm_servertime); QS("tags", w.lqm_tags); QS("target", w.lqm_target); QS("userguid", w.lqm_userguid); QS("width", w.lqm_width); QS("formatString", w.lqm_formatString); QS("ypos", w.lqm_ypos); QS("zone", w.lqm_zone); QS("ct", w.lqm_total_ads); QS("avoidTypes", w.lqm_avoidTypes);QS("renderjson",w.lqm_renderjson); QS("u", encodeURIComponent(w.location.href)); document.write('<script type="text/javascript" src="' + w.lqm_url + '"></' + 'script>'); } window.lqm_ypos = -1; CI(); BU(); RP();
})()